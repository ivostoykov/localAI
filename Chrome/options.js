...
async function mainMenuSwitched(section){
  if(!section){
    console.error(`>>> ${manifest.name} - [${getLineNumber()}] - Expected section is null or missing - ${section}`);
    return;
  }
  ...
  switch (section) {
      ...
      case 'general':
          break;
      default:
          console.error(`>>> ${manifest.name} - [${getLineNumber()}] - error: Wrong section Id - ${section}!`);
          break;
  }
}
